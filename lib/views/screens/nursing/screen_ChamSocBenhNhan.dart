...
import 'package:app_leohis/views/screens/nursing/widgets/container_ThongTinBN_CS.dart';
...
class Screen_ChamSocBenhNhan extends StatefulWidget {
  const Screen_ChamSocBenhNhan({super.key});

  @override
  State<Screen_ChamSocBenhNhan> createState() => _Screen_ChamSocBenhNhanState();
}

enum SingingCharacter { jefferson, adams }

class _Screen_ChamSocBenhNhanState extends State<Screen_ChamSocBenhNhan> {
  //Data
  List<BenhNhanCSData> _list = [];
  //Fillter
  List<BenhNhanCSData> _displayList = [];
  ...
  void setListBenhNhan(BuildContext context) async {
    ...
    _list = await _khoaProvider.listBenhNhanChamSoc;
            ...
            SpeedDialChild(
              ...
              onTap: () {
                setState(() {
                  ...
                  isDescDate
                      ? _displayList.sort(
                          (a, b) => a.ngayvaovien.compareTo(b.ngayvaovien))
                      : _displayList.sort(
                          (a, b) => b.ngayvaovien.compareTo(a.ngayvaovien));
                });
              },
            ),
            // SpeedDialChild(
        ...
        child: Container(
          child: Column(
            children: [
              ...
              //List bệnh nhân
              DividerTitle(title: "Danh sách chăm sóc  (${_list.length})"),
              loading
...
class CardItem extends StatelessWidget {
  const CardItem({
    super.key,
    required BenhNhanCSData data,
  }) : _data = data;

  final BenhNhanCSData _data;
              ...
              Row(
                ...
                children: [
                  ...
                  Expanded(
                    ...
                    child: Row(
                      children: [
                        Icon(UniconsLine.document_layout_center, size: 20),
                        SizedBox(width: 10),
                        Expanded(
                          child: Text("${_data.chandoanTaikhoa}",
                              maxLines: 1, overflow: TextOverflow.ellipsis),
                        ),
                      ],
                    ),
